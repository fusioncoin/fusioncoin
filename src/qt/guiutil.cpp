...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("fusioncoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("fusioncoin://"))
    {
        uri.replace(0, 13, "fusioncoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Fusioncoin.lnk";
}
...
{
    return GetAutostartDir() / "fusioncoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Fusioncoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  fusioncoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Fusioncoin-Qt"));
